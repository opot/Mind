...
public abstract class Mob extends GameObject {
	...
	public void draw(SpriteBatch batch, Sprite world_mask, GamePlayState game) {
		...
		float x = (float) ((game.world.radius + anim.getHeigth()) * Math.cos(Math.toRadians(angle))) - hp_mask.getWidth()/2;
		if (x > -500 && x < 500) {
			float y = (float) ((game.world.radius + anim.getHeigth()) * Math.sin(Math.toRadians(angle)))+ game.world.y / 2;
			hp_mask.setPosition(x, y);
			if(hp_mask.getWidth()-hp*2>0)
				hp_mask.setSize(hp_mask.getWidth()-Gdx.graphics.getDeltaTime()*500,hp_mask.getHeight());
